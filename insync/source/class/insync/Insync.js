 ...
 */
qx.Class.define("insync.Insync",
{
  extend : qx.core.Object,

  construct : function(name, target, callback, context)
  {
    this.base(arguments);
    ...
    // Create and save the proxy
    this.setProxy(
      this.constructor._ObservableSlim.create(
        target,
        false,
        this._handler.bind(this)));

    // Initially use the null publisher. The user can change it and the target
    // will be fully published.
    this.setPublisher(this._nullPublisher);
  },

  properties :
  {
    /** The proxy object created for this instance */
    proxy :
    {
      check    : "Proxy",
      nullable : true,
    ...
    /**
     * Function to publish all changes to.
     * @signature (String name, String type, Object data, Number serial)
     */
    publisher :
    {
      check    : "Function",
      nullable : false,
      init     : null,
      apply    : "_applyPublisher"
    }
  },

  members :
  {
    /** Name of this insync object, for coordination with listening peers */
    _name     : null,

    /** The local target object */
    _target   : null,

    /** Serial number sent with each published message */
    _serial   : 0,

    /**
     * User's callback function.
     * @signature (String name, Map[] changes)
     */
    _callback : null,

    /** Context in which User's callback function will be called */
    _context  : null,

    destroy : function()
    {
      this.constructor._ObservableSlim.remove(this.getProxy());
      this.setProxy(null);

      // Publish this removal
      this.getPublisher()(this._name, "removal", ++this._serial);
    },

    /**
     * Receives each observed change on the proxy. If a callback function was
     * registered to receive changes, it will be called with a slightly
     * altered (to use qooxdoo-like naming conventions) set of change values.
     *
     * @param {Array} changes
     *   The list of changes that have occurred.
     */
    _handler : function(changes)
    {
      // Munge the internal data into more standard qooxdoo-style
      // naming. Strip off `target` as it should always contain the same data
      // as `proxy`. That data (`target`/`proxy`) is named `data` here. (User
      // handlers that ship this data over a network may want to delete `data`
      // and send only the remaining change information, since `data` could
            ...
            return (
              {
                ...
                old      : change.previousValue,
                data     : this._target
              });
          });

      // Call the user's callback, if one was provided
      if (this._callback)
      {
        this._callback.call(this._context || null, this._name, changes);
      }

      // Publish this change set
      this.getPublisher()(this._name, "changes", changes, ++this._serial);
    },

    /**
     * Function called when publishing is requested. This one doesn't do
     * anything. Change the `publisher` property to a function with this same
     * signature, to do something more meaningful.
     *
     * @param {String} name
     *   The proxy's name.
     *
     ...
     */
    _nullPublisher : function(name, type, data, serial)
    {
      console.log(
        `Null publisher: type=${type} name=${name}, serial=${serial} data=\n` +
          JSON.stringify(data, null, "  "));
    },

    _applyPublisher : function(value, old)
    {
      // Publish the initial data
      this.getPublisher()(this._name, "data", this._target, ++this._serial);
    }
  },
  ...
  defer : function(statics)
  {
          ...
          var handler = {
            ...
            deleteProperty: function(target, property) {
              ...
              changes.push({
                "type":"delete"
                ,"target":target
                ,"parent":path.map(o => o.property).slice(1) // djl added
                ,"property":property
                ,"newValue":null
    ...
    statics._ObservableSlim = ObservableSlim;
  }
});
